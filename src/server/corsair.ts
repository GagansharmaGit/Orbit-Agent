...
export async function syncCorsairTokens(session: any) {
  ...
  try {
    ...
    // 1. Global Setup (Run exactly once globally)
    if (!globalKeysSynced) {
    ...
    // 2. Tenant Setup (Run exactly once per tenant)
    if (!syncedTenants.has(tenantId)) {
      if (!tenantSyncPromises.has(tenantId)) {
        tenantSyncPromises.set(tenantId, (async () => {
    ...
    tenantSyncPromises.delete(tenantId);
  }
}
